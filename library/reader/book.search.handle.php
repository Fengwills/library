...
$page = $_GET['p'];
$page_size = 5;
$Book_name = $_GET['query_bname'];
$category = $_GET['query_cname'];
$author = $_GET['query_author'];
$publishing = $_GET['query_publish'];
$sql = "select * from book_info WHERE Book_name LIKE '%$Book_name%'and category LIKE '%$category%' AND author LIKE '%$author%' AND publishing LIKE '%$publishing%'order BY Book_id LIMIT ".($page-1)*$page_size .",$page_size";
$query = mysqli_query($con,$sql);
$total_sql = "select * from book_info WHERE Book_name LIKE '%$Book_name%'and category LIKE '%$category%' AND author LIKE '%$author%' AND publishing LIKE '%$publishing%'order BY Book_id ";
                                    ...
                                    <ul class="pagination">
                                        ...
                                        if($page>1)
                                        {
                                            $page_banner="<li class='paginate_button previous' id='data_list_previous'><a
                                                href=".$_SERVER['PHP_SELF']."?p=".($page-1)."&query_bname=".$Book_name."&query_cname=".$category."&query_author=".$author."&query_publish=".$publishing."
                                            >上一页</a></li>";
                                            echo "$page_banner";
                                        }
                                        if($page<$total_page)
